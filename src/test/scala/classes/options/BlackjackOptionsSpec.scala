...
class BlackjackOptionsSpec extends AnyFlatSpec with GivenWhenThen {
  ...
  it should "be able to deserialize a json string which specifies an initial betting strategy of Martingale" in {
    Given("JSON string which specifies optional initial betting strategy of Martingale")
    val json: String = """{"initial-betting-strategy":"Martingale","dealerHitLimit":"S17","blackjackPayout":"ThreeToTwo","deckCount":"1","splitLimit":"3","allowSurrender":"true","hitOnSplitAces":"true","resplitOnSplitAces":"true","initialBank":"2000","playerInitialRanks":["Queen","Queen"],"dealerInitialRanks":["King","Ace"]}""";
    When("converting the JSON string into a blackjack options object")
    val result = BlackjackOptions(json)
